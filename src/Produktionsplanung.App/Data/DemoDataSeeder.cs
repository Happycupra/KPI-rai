...
public static class DemoDataSeeder
{
    ...
    {
        // Demo-/Startdaten dürfen ausschliesslich eine wirklich leere Erstinstallation befüllen.
        // Bestehende Datenbanken werden bewusst nicht "repariert" oder nachgefüllt: fehlende
        // Schichtregeln können eine absichtliche Deaktivierung sein, und leere Auftragslisten
        // können vom Anwender bewusst erzeugt worden sein.
        if (ShouldCreateInitialDemoData(db))
        {
            SeedBaseData(db);
            EnsureInitialWorkstationShiftRules(db);
    ...
    private static bool ShouldCreateInitialDemoData(AppDbContext db) =>
        !db.Employees.Any() &&
        !db.Workstations.Any() &&
        !db.Shifts.Any() &&
        !db.Qualifications.Any() &&
        !db.ProductionOrders.Any() &&
        !db.UserAccounts.Any();
    ...
    private static void EnsureInitialWorkstationShiftRules(AppDbContext db)
    {
        var workstations = db.Workstations.ToList();
        var shifts = db.Shifts.ToList();
        var existing = db.WorkstationShiftRules
            .Select(x => new { x.WorkstationId, x.ShiftId })
            .ToList();

        foreach (var workstation in workstations)
        foreach (var shift in shifts)
        {
            if (existing.Any(x => x.WorkstationId == workstation.Id && x.ShiftId == shift.Id))
                continue;
    ...
    private static void SeedProductionOrders(AppDbContext db)
    {
        ...
        if (line1 is null || line2 is null || early is null || late is null)
            return;
